...
ブラウザ画面を表示せずにバックグラウンドで動作するモードです。ログインが必要な場合は、ターミナル上でメールアドレスと認証コードを入力します。サーバー環境やCI/CDパイプラインでの自動実行に適しています。ただし、OAuth認証（GitHub/Google等）には対応していないため、メール＋認証コード方式でのログインのみ利用可能です。
